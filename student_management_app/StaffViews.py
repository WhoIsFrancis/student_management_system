...
from student_management_app.models import Attendance, AttendanceReport, CustomUser, FeedBackStaffs, LeaveReportStaff, SessionYearModel, Staffs, Students, Subjects

def staff_home(request):
    return render(request,'staff_template/staff_home_template.html')

def staff_take_attendance(request):
    #Fetch de todos los Subjects de Staffs
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    #Fetch de todo las SessionYearModels
...
def staff_apply_leave_save(request):
    if request.method != "POST":
        ...
        try:
            ...
            leave_report.save()
            messages.success(request, "Succesfully applied for leave")
            return HttpResponseRedirect(reverse("staff_apply_leave"))
        except:
